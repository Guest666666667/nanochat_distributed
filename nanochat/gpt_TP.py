...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx, mp_group):
        super().__init__()
    ...
    def forward(self, x, cos_sin, kv_cache):
        ...
        local_n_head = self.n_head // torch.distributed.get_world_size(group=self.mp_group)
...
class MLP(nn.Module):
    def __init__(self, config, mp_group):
        ...
        self.c_fc = LinearLayer(c_fc_linear, mp_group=mp_group, name='c_fc')
        self.c_proj = LinearAllreduce(c_proj_linear, mp_group=mp_group, name='c_proj')
...
class Block(nn.Module):
    def __init__(self, config, layer_idx, mp_group):
        super().__init__()
        self.attn = CausalSelfAttention(config, layer_idx, mp_group)
        self.mlp = MLP(config, mp_group)
...
class GPT(nn.Module):
    def __init__(self, config, mp_group):
        ...
        self.transformer = nn.ModuleDict({
            ...
            "h": nn.ModuleList([Block(config, layer_idx, mp_group) for layer_idx in range(config.n_layer)]),
        })
