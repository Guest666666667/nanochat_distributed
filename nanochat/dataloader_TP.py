...
def tokenizing_distributed_data_loader(TP, B, T, split, tokenizer_threads=4, tokenizer_batch_size=128, device="cuda"):
    ...
    dp_rank = ddp_rank // TP
    dp_world_size = ddp_world_size // TP
    def document_batches():
        while True:
            # batch will iterate in group size of the parquet files, usually e.g. 1024 rows
            for batch in parquets_iter_batched(split=split, start=dp_rank, step=dp_world_size):
